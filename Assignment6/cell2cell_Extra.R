...
head(cell2cell)  # check to make sure your data was read in correctly
...
heat.tree(ctree, low.is.green=TRUE, type=4, varlen=0, faclen=0, fallen.leaves=TRUE, cex=.8)
heat.tree(ctree, low.is.green=TRUE, varlen=0, faclen=0, fallen.leaves=TRUE, branch.type=5, yesno=FALSE, cex=.7)  # width proportional to # of observations
...
# !! must click "Quit" in your plot window after executing next line !!
...
#@graph models
###############################################################################
### overlay the predictions from the models against the actual data
###############################################################################

# this portion uses ggplot
if (!require(ggplot2)) {install.packages("ggplot2"); library(ggplot2)}
if (!require(randomForest)) {install.packages("randomForest"); library(randomForest)}

# (treeA) complex tree model
mymodel.treeA = rpart(Churn~., data=cell2cell[trainsample,], control=rpart.control(cp=0.005))
pchurn.treeA = predict(mymodel.treeA,newdata=cell2cell,type='vector')

# (treeB) complex tree model
mymodel.treeB = rpart(Churn~., data=cell2cell[trainsample,], control=rpart.control(cp=0.00090890))
pchurn.treeB = predict(mymodel.treeB,newdata=cell2cell,type='vector')

# (lrA) simpler logistic regression with 10 terms: stepwise model with . and steps=10
mymodel.lrA = glm(Churn~Eqpdays+Retcall+Months+Refurb+Uniqsubs+Mailres+Overage+Mou+Creditde+Actvsubs,data=cell2cell[trainsample,],family='binomial')
pchurn.lrA = predict(mymodel.lrA,newdata=cell2cell,type='response')

# (lrB) more complex logistic regression with 20 terms and interactions: stepwise model with ^2 and steps=20
mymodel.lrB = glm(Churn~Eqpdays+Retcall+Months+Refurb+Uniqsubs+Mailres+Overage+Mou+Setprcm+Creditde+Actvsubs+Roam+Changem+Changer+Marryno+Age1+Eqpdays:Months+Months:Mou+Creditde:Changem+Overage:Age1,data=cell2cell[trainsample,],family='binomial')
pchurn.lrB = predict(mymodel.lrB,newdata=cell2cell,type='response')

# estimate random forest
# !! change ntree=20 to larger values, but can take lots of time !!
# !! nodesize=30 limits branches to at least 30 observations !!
# caution: can take ~5 minutes to train using the following settings  (if time permits increase ntree)
mymodel.rf = randomForest(Churn~.,data=cell2cell[trainsample,],ntree=20,proximity=TRUE,importance=TRUE,nodesize=30)
pchurn.rf = predict(mymodel.rf,newdata=cell2cell,type='response')

# create a new dataset with the original data and the churn
cell2cell.join=cbind(cell2cell,pchurn.lrA,pchurn.lrB,pchurn.treeA,pchurn.treeB,pchurn.rf)[!testsample,]

# plot actual data with a smoothed line
b = ggplot(cell2cell.join, aes(x=Eqpdays, y=Churn)) +
  geom_point(position='jitter',alpha=1/20) + geom_smooth(span=.01)
b  # print graph now

# overlay the smoothed prediction line for each model
b +  
  geom_smooth( aes(x=Eqpdays, y=pchurn.lrA, colour="green"), span=.01) +
  geom_smooth( aes(x=Eqpdays, y=pchurn.lrB, colour="darkgreen"), span=.01) +
  geom_smooth( aes(x=Eqpdays, y=pchurn.treeA, colour="purple"), span=.01) +
  geom_smooth( aes(x=Eqpdays, y=pchurn.treeB, colour="magenta"), span=.01) +
  geom_smooth( aes(x=Eqpdays, y=pchurn.rf, colour="red"), span=.01) +
  scale_colour_manual(
    values=c("green","darkgreen","purple","magenta","red"),
    labels=c("lrA","lrB","treeA","treeB","rf"),name="")
