...
setwd("~/MA") #!! set to your directory
...
forddemo=read.csv("data/FordKaDemographicData.csv",row.names=1)  # just the demographic data
fordpsyc=read.csv("data/FordKaPsychographicData.csv",row.names=1)  # just the psychographic data
#fordquest=scan("data/FordKaQuestions.txt",what='a',sep='\n')  # question list, which is read as a vector
fordquest = scan("data/FordKaQuestions.txt",what = list(""),sep='\n')
...
nshortqlist = c(2,41,44,17,14,31)
...
#balloonplot(table(ford$FirstTimePurchase,ford$IncomeCategory),xlab="FirstTimePurchase",ylab="IncomeCategory")
...
###############################################################################
## first cluster analysis with psychographics
###############################################################################
...
(grpA=kmeans(fordpsyc[,qlist],centers=4,nstart=100))
...
plot(jitter(fordpsyc[,"Q1"]), jitter(fordpsyc[,"Q2"]),xlab=fordquest[1],ylab=fordquest[2],col=grpA$cluster)
points(grpA$centers[,c("Q1","Q2")],col=1:4,pch=8,cex=2)
legend("topleft",pch=8,bty="n",col=1:4,c("1","2","3","4"))
...
variances = RowVar(grpAcenter)

rownames(grpAcenter)=afordquest  # add the question names
print(grpAcenter)   # print the centroid values for each question
parallelplot(t(grpAcenter),auto.key=list(text=c("1","2","3","4"),space="top",columns=4,lines=T))  # create a parallel plot to visualize the centroid values
parallelplot(t(grpAcenter[nshortqlist,]),auto.key=list(text=c("1","2","3","4"),space="top",columns=4,lines=T))  # a parallel plot with just a few questions
...
###############################################################################
### second cluster analysis with demographics
###############################################################################
...
(grpB=kmeans(xford[,qdlist],centers=5,nstart=100))

# plot the solutions against Age and FirstTimePurchase
# since the data is categorical most of the plots will overlay one another,
# so instead we jitter the points -- which adds a small random number to each
par(mfrow=c(1,1),mar=c(5,4,4,1))
plot(jitter(xford[,"MaritalStatus"]), jitter(xford[,"Gender"]),xlab="MaritalStatus",ylab="Gender",col=grpB$cluster)
points(grpB$centers[,c("MaritalStatus","Gender")],col=1:5,pch=8,cex=2)
legend("topleft",pch=8,bty="n",col=1:5,c("1","2","3","4","5"))
